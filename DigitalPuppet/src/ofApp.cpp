...
//	ofSetLogLevel(OF_LOG_VERBOSE);
    ...
    ofxObjLoader::load("head_top.obj", top, false);
    ofxObjLoader::load("head_bottom.obj", bottom, false);
}
...
//	ofPopMatrix();
    ...
    for(int i = 0; i < simpleHands.size(); i++){
        ...
        float dist = simpleHands[i].fingers[THUMB].tip.distance(simpleHands[i].fingers[MIDDLE].tip);
        ...
        ofEnableDepthTest();
        light.enable();
        light.setPosition(ofGetWidth()/2.0, ofGetHeight()/2.0, 50);
        ofTranslate(handPos);
